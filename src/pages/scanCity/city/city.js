...
class City {
  constructor(p) {
        ...
        // 需要做城市效果的mesh
        const cityArray = ['CITY_UNTRIANGULATED','Body002', 'Skeletal001', 'Circulatory_Heart001'];

        const floorArray = ['LANDMASS'];

        this.loadFbx('./img/demo01.fbx').then((scene) => {
            ...
            scene.traverse((child) => {
              // 城市效果
                if (cityArray.includes(child.name)) {
                    ...
                    // 添加包围线条效， 也是在这里进行的水平扫光效果，（只是改变了包围线条的颜色）
    ...
    setCityMaterial(object) {
      ...
      Utils.forMaterial(object.material, (material) => {
        ...
        material.onBeforeCompile = (shader) => {
          ...
          // 效果颜色
          shader.uniforms.uColor = {
            value: new THREE.Color("#17d47a")
          }
          // 效果颜色
          shader.uniforms.uFlowColor = {
            value: new THREE.Color("#20aa51")
          }
    ...
    float indexMix = vPosition.z / (uSize.z * 0.9);
    distColor = mix(distColor, uTopColor, indexMix);
    ...
    if (uDiffusion.x > 0.5) {
        // 扩散速度
        float dTime = mod(time * uDiffusion.z, uRadius * 1.0);
        // 当前的离中心点距离
        float uLen = distanceTo(position2D, vec2(uCenter.x, uCenter.z));
